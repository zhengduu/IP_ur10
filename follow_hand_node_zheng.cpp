...
/*
* modified version by Zheng Du, Sep 2018
* added functions:
* - transforming base_link of UR10 directly onto RightShoulder of
*   tf skeleton in the simulation
* - different lenghts of UR10 and tf skeleton taken into account
*   and added a scaling factor
*/
...
int pushIntoTransformArray(std::string transOrigin, std::string transDestination,
...
struct tfPercNeuronInterface {
  ...
  tf::Transform transBaseLinkRightHand;    // goal Transform for IK calculations,
                                           ...
                                           // pushIntoTransformArray directly
  tf::TransformBroadcaster tfBroadcaster;  // broadcasts goal transform to tf & visualize in rviz
  ...
  int getTransBaseLinkRightHand() {
    tf::Transform
        transRightShoulderRightHand; // transform from RightShoulder to RightHand
    std::vector<tf::Transform> transformArray;
    int success =
        0;  // = 0 for success of getTransBaseLinkRightHand; < 0 for failure

    // Get Transform from RightShoulder to RightHand of PercNeuron
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    // fill transformArray with transforms from "RightShoulder" to "RightHand"; i=0
    // would equal RightShoulder
    // for loop could be removed and "RightShoulder" to "RightHand" can be directly
    // transformed for transBaseLinkRightHand
    ...
    // Calculate direct vector from RightShoulder to RightHand

    // multiplying transform matrices from RightShoulder to RightHand
    transRightShoulderRightHand = transformArray.at(0);
    for(int i = 1; i < transformArray.size(); i++){
        transRightShoulderRightHand = transRightShoulderRightHand.operator*=(
                    transformArray.at(i));
    }

    // calculate total lenght of right arm of tf_skeleton
    double tf_skeleton_armlenght = 0.0;
    // only X values of translational part needed (check tf_echo)
    for (int i = 0; i < transformArray.size(); i++ ){
        tf_skeleton_armlenght = tf_skeleton_armlenght + fabs(transformArray.at(i).
                                                        getOrigin().getX());
    }
    ...
    // directly getting transRightShoulderRightHand after getting transform of
    // base_link to RightShoulder
    transBaseLinkRightHand = transRightShoulderRightHand;
    ...
    // scale factor is ratio of lenght of robot arm to right arm
    // of the tf skeleton from the Axis Neuron model
    double scaling_factor = 0.0;
    // lenght of robot arm is calculated with Z values of translational part
    // absolute values for calculation of scaling factor
    scaling_factor = 1.43 / (tf_skeleton_armlenght);

    // scaling the translational part of transBaseLinkRightHand
    transBaseLinkRightHand.getOrigin() =
            transBaseLinkRightHand.getOrigin() * scaling_factor;

    // Broadcast the transform from RightShoulder to the RightHand
    tfBroadcaster.sendTransform(tf::StampedTransform(
          transRightShoulderRightHand, ros::Time::now(),
                                    "/RightShoulder", "/RightShoulderToRightHand"));

    // Broadcast the transform from base_link to the EE
    tfBroadcaster.sendTransform(tf::StampedTransform(
          transBaseLinkRightHand, ros::Time::now(),
...
int checkAndPublishDesJointValues(ros::NodeHandle& node,
  ...
  int elbowJointIndex = 2;
          // Index of elbow joint in goal/solution/delta/lastPubPosition vector
...
int main(int argc, char** argv) {
  ...
  // provided by the Perception Neuron API, to calculate goal transform from
  // RightShoulder to RightHand
  ...
  tf::TransformBroadcaster br;  // new transform from RightShoulder to base_link
  tf::Transform transRightShoulderBaseLink;
  transRightShoulderBaseLink.setOrigin(tf::Vector3(0.0, 0.0, 0.0));
  transRightShoulderBaseLink.setRotation(tf::Quaternion(0, 0, 0, 1));
  // transform base_link onto RightShoulder instead of WorldPerceptionNeuron
  br.sendTransform(tf::StampedTransform(transRightShoulderBaseLink, ros::Time::now(),
                                        "RightShoulder", "base_link"));
